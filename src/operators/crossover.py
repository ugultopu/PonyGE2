...
def crossover(parents):
    ...
    while len(cross_pop) < params['GENERATION_SIZE']:
        ...
        # If using clustering, make sure that the parents are not from the same
        # cluster
        if params['SELECTION'].__name__ == 'clustering':
            while inds_in[0][1] == inds_in[1][1]: inds_in = sample(parents, 2)
            # Omit the cluster part from the parents once two parents from
            # different clusters are selected.
            inds_in = [ i[0] for i in inds_in ]
