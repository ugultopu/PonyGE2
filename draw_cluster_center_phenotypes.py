...
def draw_phenotype(cluster_idx, phenotype_idx, phenotype):
    turtle.reset()
    for move in phenotype:
        if move == 'F':
            ...
            turtle.right(90)
    turtle.getscreen().getcanvas().postscript(file='phenotype_'+str(cluster_idx)+'_'+str(phenotype_idx)+'.eps')
...
for cluster_idx, phenotype_list in enumerate(phenotypes_for_clusters):
    for phenotype_idx, phenotype in enumerate(phenotype_list):
        draw_phenotype(cluster_idx, phenotype_idx, phenotype)
